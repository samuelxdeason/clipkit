...
export default function VideoCard({ video: v, selected, selectionMode, thumbnail, onSelect, onPlay, onTag }: { video: library.Video; selected: boolean; selectionMode: boolean; thumbnail: string; onSelect: (shift: boolean) => void; onPlay: () => void; onTag: (tag: string) => void }) {
  ...
  return <article className={`video-card ${portrait ? "is-portrait" : ""} ${selected ? "is-selected" : ""} ${selectionMode ? "is-selecting" : ""}`}>
    ...
    <div className="video-card-body"><button className="video-card-title" title={title} onClick={activate} onDoubleClick={() => { if (selectionMode) onPlay(); }}>{title}</button><p>{v.site}{v.height ? ` · ${v.height}p` : ""}</p><div className="tag-list">{(v.labels || []).slice(0, 3).map(t => <button className="tag-pill" key={t} onClick={() => onTag(t)}>{t}</button>)}</div></div>
  </article>;
}
