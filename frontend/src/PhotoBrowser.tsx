import { LibraryToolbar, LoadState } from "./WorkspaceUI";
...
export default function PhotoBrowser({ person = "", query, onQuery, searchRef, media, version, onChanged, importOpen, onImportClose }: {
  ...
  useEffect(() => {
    ...
    })().catch(e => { if (live) setLoadError(String(e)); }).finally(() => { if (live) setLoading(false); });
    return () => { live = false; };
  ...
  useEffect(() => { setSelected(current => new Set([...current].filter(id => filtered.some(p => p.id === id)))); }, [filtered]);
  const openOrSelect = (id: string) => {
    if (busy) return;
    if (!selectMode) { setActiveID(id); return; }
  ...
  const trashSelected = async () => {
    ...
    try {
      await api.TrashMedia([], ids);
      setPhotos(current => current.filter(p => !ids.includes(p.id)));
      setActiveID(null); setSelected(new Set()); setSelectMode(false);
      setNotice(`${ids.length} photo${ids.length === 1 ? "" : "s"} moved to your vault’s Recycle bin folder.`);
      onChanged();
  ...
  return <section ref={section} className="photo-library">
    ...
    <LibraryToolbar className="video-browser-controls photo-browser-controls" label="Photo library controls">
      ...
      <div className="video-result-scope"><span>{loading ? "Loading photos…" : loadError ? "Photos unavailable" : `${filtered.length.toLocaleString()} photo${filtered.length === 1 ? "" : "s"}`}{query ? " matching your search" : ""}</span></div>
    </LibraryToolbar>
    <div className="person-content-toolbar">
      <button className="m-button" disabled={busy || loading || !filtered.length} aria-pressed={selectMode} onClick={() => { setSelectMode(mode => !mode); setSelected(new Set()); }}>{selectMode ? "Cancel selection" : "Select photos"}</button>
      {selectMode && <button className="m-button" disabled={busy} onClick={() => setSelected(new Set(selected.size === filtered.length ? [] : filtered.map(p => p.id)))}>{selected.size === filtered.length ? "Deselect all" : "Select all matching"}</button>}
    </div>
    {selectMode && <div className="bulk-bar" role="toolbar" aria-label="Bulk photo actions">
      <strong aria-live="polite">{selected.size} selected</strong>
      <button disabled={busy || !selected.size} onClick={trashSelected}>{busy ? "Moving…" : "Move to trash"}</button>
      <span>Files move to your vault’s Recycle bin folder.</span>
    </div>}
    ...
    {loading ? <LoadState title="Opening your photos…" busy /> : loadError ? <LoadState title="Couldn’t load your photos" message={loadError} onRetry={() => setRetry(n => n + 1)} /> : rows.length ? layout === "grid" ? <div className="manager-grid photo-library-grid">{rows.map(p => <button key={p.id} onClick={() => openOrSelect(p.id)} className={`photo-tile ${selected.has(p.id) ? "photo-selected" : ""}`} aria-pressed={selectMode ? selected.has(p.id) : undefined} aria-label={(selectMode ? (selected.has(p.id) ? "Deselect " : "Select ") : "Open ") + p.filename}><span className="photo-tile-frame"><img src={media(p.filepath)} alt="" loading="lazy" /></span><strong title={p.filename}>{selectMode && <span aria-hidden>{selected.has(p.id) ? "✓ " : "○ "}</span>}{p.filename}</strong><small>{p.album || p.model || "Photo"}</small></button>)}</div> : <div className="table-scroll"><table className="file-table photo-file-table"><thead><tr><th>Name</th><th>Album</th><th>Person</th><th>Date added</th></tr></thead><tbody>{rows.map(p => <tr key={p.id} tabIndex={0} className={selected.has(p.id) ? "selected-row" : ""} aria-selected={selectMode ? selected.has(p.id) : undefined} onClick={() => openOrSelect(p.id)} onKeyDown={e => { if (e.target === e.currentTarget && (e.key === "Enter" || e.key === " ")) { e.preventDefault(); openOrSelect(p.id); } }}><td><button className="file-name" onClick={e => { e.stopPropagation(); openOrSelect(p.id); }}><span className="file-thumb"><img src={media(p.filepath)} alt="" loading="lazy" /></span><strong>{selectMode && <span aria-hidden>{selected.has(p.id) ? "✓ " : "○ "}</span>}{p.filename}</strong></button></td><td>{p.album || "—"}</td><td>{p.model || "—"}</td><td>{date(p.added)}</td></tr>)}</tbody></table></div> : <div className="manager-empty"><Icon name="photo" /><h2>{photos.length ? "No matching photos" : "A home for your photos"}</h2><p>{photos.length ? "Try another search, album, or view." : "Use Add photos to import from your device or a gallery link."}</p></div>}
