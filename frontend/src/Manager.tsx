import { LibraryToolbar, LoadState, PageHeader } from "./WorkspaceUI";
...
import { PersonAccounts } from "./PhotoLibrary";
...
type View = "duplicates" | "following" | "photos" | "profile" | "all" | "recent" | "unorganized" | "favorites" | "people" | "tags" | "collections" | "downloads" | "settings";
...
const titles: Record<View, string> = { duplicates: "Potential duplicates", following: "Following", photos: "Photos", profile: "Person", all: "All videos", recent: "Recently added", unorganized: "Needs organizing", favorites: "Favorites", people: "People", tags: "Tags", collections: "Collections", downloads: "Downloads", settings: "Settings" };
...
export default function Manager() {
  ...
  const rememberDialogTrigger = () => { const active = document.activeElement as HTMLElement | null; if (active && !active.closest(".manager-modal")) dialogTrigger.current = active; };
  ...
  const photoView = view === "photos" || view === "profile" && profileTab === "photos";
  useEffect(() => { setPhotoImportOpen(false); }, [view, profileTab]);
  ...
  useEffect(() => {
    ...
    setLoadError("");
    if (loadedScope.current !== scope) { loadedScope.current = null; setLoading(true); setVideos([]); }
    (async () => {
    ...
    })().catch(e => { if (!cancelled) setLoadError(String(e)); }).finally(() => { if (!cancelled) setLoading(false); });
  ...
  const openCollection = (c: library.Collection) => { if (c.locked) { rememberDialogTrigger(); setGate(c); return; } go("all"); setCollection(c); };
  const openModal = (m: typeof modal, target?: Video) => { rememberDialogTrigger(); setEditTarget(target || null); setValue(""); setModal(m); if (m === "tag") api.AllLabels().then(t => setSuggestedTags(t || [])).catch(e => setError(String(e))); };
  ...
  const fileView = !["duplicates", "photos", "people", "tags", "collections", "downloads", "settings", "following"].includes(view) && !(view === "profile" && ["photos", "accounts"].includes(profileTab));
  ...
  return <div className="manager finder-shell">
    <aside id="workspace-navigation" className={`manager-sidebar ${nav ? "is-open" : ""}`}>
      ...
      {([["collections", "folder"], ["people", "people"], ["tags", "tag"]] as [View, string][]).map(([id, icon]) => <button key={id} className={`manager-nav ${(view === id || id === "people" && view === "profile") ? "active" : ""}`} data-section={id} aria-current={(view === id || id === "people" && view === "profile") ? "page" : undefined} onClick={() => go(id)}><Icon name={icon} />{titles[id]}</button>)}
      ...
      <div className="sidebar-bottom"><nav className="sidebar-manage" aria-label="Manage library"><div className="nav-label">Manage</div><button className={`manager-nav ${view === "following" ? "active" : ""}`} data-section="following" aria-current={view === "following" ? "page" : undefined} onClick={() => go("following")}><Icon name="connections" />Following</button><button className={`manager-nav ${view === "downloads" ? "active" : ""}`} data-section="downloads" aria-current={view === "downloads" ? "page" : undefined} onClick={() => go("downloads")}><Icon name="download" />Downloads<span className="nav-count">{queue.filter(j => j.status === "queued" || j.status === "downloading").length || ""}</span></button><button className={`manager-nav ${view === "duplicates" ? "active" : ""}`} aria-current={view === "duplicates" ? "page" : undefined} onClick={() => go("duplicates")}><Icon name="film" />Potential duplicates</button></nav><div className="sidebar-divider" /><button className={`manager-nav ${view === "settings" ? "active" : ""}`} data-section="settings" aria-current={view === "settings" ? "page" : undefined} onClick={() => go("settings")}><Icon name="gear" />Settings</button></div>
    </aside>
    <div className="manager-workspace">
      <header className="manager-topbar"><div className="chrome-row"><button className="mobile-nav" aria-label="Toggle navigation" aria-expanded={nav} aria-controls="workspace-navigation" onClick={() => setNav(!nav)}><Icon name="menu" /></button><nav className="breadcrumbs" aria-label="Breadcrumb"><ol>{crumbs.map((crumb, index) => <li key={index}>{index > 0 && <svg className="breadcrumb-chevron" viewBox="0 0 16 16" aria-hidden="true"><path d="m6 4 4 4-4 4" /></svg>}{index === crumbs.length - 1 ? <strong aria-current="page" title={crumb.label}>{crumb.label}</strong> : <button onClick={crumb.onClick} title={crumb.label}>{crumb.label}</button>}</li>)}</ol></nav>{fileView && view !== "profile" && !videoLibrary && fileControls}{photoView && <button className="m-button library-add" onClick={() => setPhotoImportOpen(true)}><Icon name="plus" />Add photos</button>}{videoLibrary && <button className="m-button library-add" onClick={() => openModal("import")}><Icon name="plus" />Add videos</button>}</div>{fileView && view !== "profile" && !videoLibrary && filters && filterPanel}</header>
      <main key={pageKey} ref={mainRef} className={`manager-main view-${view} ${fileView ? "file-view" : ""}`}>
        {view === "profile" && profile ? <PersonHeader key={profile.name} person={people.find(p => p.name === profile.name) || profile} videos={videos.filter(v => v.people?.includes(profile.name))} media={media} onChanged={reload} /> : fileView || photoView ? <h1 className="sr-only">{collection?.name || titles[view]}</h1> : <PageHeader title={titles[view]} description={({ following: "Follow sources, check for new content, and download entire collections.", people: "People and their connected media.", collections: "Keep related videos together.", tags: "Browse your library by tag.", photos: "Your photos, organized by album.", downloads: "Add links and manage your download queue.", settings: "Manage your library and preferences." } as Partial<Record<View, string>>)[view]} actions={(view === "people" || view === "collections") && <button className="m-button" onClick={() => openModal(view === "people" ? "person" : "collection")}><Icon name="plus" />{view === "people" ? "New person" : "New collection"}</button>} />}
        ...
        {videoLibrary && <LibraryToolbar className="video-browser-controls" label="Video library controls">{videoSearchControls}{videoActions}<div className="video-result-scope"><span>{loading ? "Loading videos…" : loadError && !videos.length ? "Library unavailable" : `${filtered.length.toLocaleString()} video${filtered.length === 1 ? "" : "s"}`}{q ? " matching your search" : ""}</span>{view === "recent" && <label>Added within<select aria-label="Recently added period" value={recentDays} onChange={e => setRecentDays(Number(e.target.value))}>{[7, 30, 90].map(days => <option key={days} value={days}>{days} days</option>)}</select></label>}{view === "unorganized" && <label>Show<select aria-label="Organization filter" value={organizeMode} onChange={e => setOrganizeMode(e.target.value as OrganizeMode)}><option value="all">All incomplete</option><option value="people">Missing people</option><option value="tags">Missing tags</option></select></label>}</div></LibraryToolbar>}
        {loadError && !fileView && !photoView && !["settings", "following", "duplicates"].includes(view) && <LoadState title="Couldn’t load your library" message={loadError} onRetry={reload} />}
        {error && <div className="manager-alert" role="alert">{error}<button onClick={reload}>Retry</button></div>}
        {loadError && fileView && videos.length > 0 && <div className="manager-alert" role="alert">Couldn’t refresh your library. Showing previously loaded videos.<button onClick={reload}>Retry</button></div>}
        ...
        {view === "profile" && fileView && <LibraryToolbar className="person-content-controls" label="Person video controls">{videoSearchControls}{videoActions}</LibraryToolbar>}
        {!fileView && !photoView && !["duplicates", "settings", "downloads", "following"].includes(view) && <div className="content-search-row"><div className="manager-search"><Icon name="search" /><input ref={searchRef} aria-label="Search this view" placeholder={profileTab === "photos" ? "Search photos, albums, or filenames…" : profileTab === "accounts" && view === "profile" ? "Search this person’s accounts…" : view === "people" ? "Search people…" : "Search this view…"} value={q} onChange={e => setQ(e.target.value)} /><kbd>Ctrl K</kbd>{q && <button aria-label="Clear search" onClick={() => setQ("")}><Icon name="x" /></button>}</div></div>}
        {fileView ? <>
          ...
          {picked.length > 0 && <div className="bulk-bar" role="toolbar" aria-label="Bulk video actions"><strong>{picked.length} selected</strong><button disabled={busy} title="Move files to your vault's Recycle bin folder" onClick={() => execute(async () => { await api.TrashMedia(picked, []); setSelected(new Set()); setSelectionMode(false); }, `${picked.length} videos moved to your vault's Recycle bin folder.`)}>Move to trash</button><button disabled={busy || picked.length < 2} title="Select at least two videos" onClick={() => execute(async () => { await api.CreateDuplicateGroup(picked); setSelected(new Set()); }, "Added to Potential duplicates. Open the queue in the sidebar to review.")}>Mark as potential duplicates</button><button onClick={() => openModal("add")}><Icon name="folder" />Add to collection</button><button onClick={() => openModal("tag")}><Icon name="tag" />Add tags</button><button onClick={() => openModal("assign")}><Icon name="people" />Assign person</button>{collection && !collection.locked && <button onClick={() => execute(async () => { for (const v of picked) await api.RemoveFromCollection(collection.id, v.site, v.id); setSelected(new Set()); }, "Removed from collection")}>Remove from collection</button>}<button aria-label="Clear selection" onClick={() => setSelected(new Set())}><Icon name="x" /></button></div>}
