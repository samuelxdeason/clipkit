...
export function LibraryDownloads({ queue, onChanged, onSettings, onFollowing }: { onFollowing: () => void; queue: downloader.Job[]; onChanged: () => void; onSettings: () => void }) {
  ...
  const position = new Map(waiting.map((j, i) => [j.id, i + 1]));
  const completed = queue.filter(j => ["done", "duplicate"].includes(j.status));
  const failed = queue.filter(j => j.status === "error");
  const scopedRows = filter === "queued" ? waiting : filter === "downloading" ? downloading : filter === "completed" ? completed : filter === "error" ? failed : queue;
  ...
  return <div className="utility-page downloads-page">
    <div className="download-add-card"><div className="download-add-content"><div className="download-add-heading"><h2>New download</h2><button className="m-button" onClick={onFollowing}><Icon name="connections" />Browse Following</button></div><form onSubmit={e => { e.preventDefault(); const value = url.trim(); if (value) run("add", async () => { await api.Enqueue(value); setUrl(""); }, "Download added to the queue."); }}><input type="url" required aria-label="Media URL" placeholder="Paste a video or photo link…" value={url} onChange={e => setUrl(e.target.value)} /><button className="m-button primary" disabled={!!busy || !url.trim()}><Icon name="plus" />{busy === "add" ? "Adding…" : "Add download"}</button></form><div className="download-import"><span>Already on your device?</span><button disabled={!!busy} onClick={() => run("files", () => api.ImportFilesDialog(""), "Files submitted for import.")}><Icon name="folder" />{busy === "files" ? "Importing…" : "Import files…"}</button>{api.isDesktopApp && <button disabled={!!busy} onClick={() => run("folder", () => api.ImportFolderDialog(), "Folder submitted for import.")}>{busy === "folder" ? "Importing…" : "Import folder…"}</button>}</div></div></div>
    ...
    {notice && <div className="manager-notice" role="status">{notice}</div>}
    <div className="queue-section-title"><h2>Download queue</h2></div>
    <div className="download-queue-heading"><nav className="utility-tabs" aria-label="Download filters">{[["all", "All", queue.length], ["downloading", "Downloading", downloading.length], ["queued", "Waiting", waiting.length], ["completed", "Completed", completed.length], ["error", "Failed", failed.length]].map(([id, name, count]) => <button key={id} className={filter === id ? "active" : ""} aria-pressed={filter === id} onClick={() => setFilter(String(id))}>{name}<span>{count}</span></button>)}</nav></div>
    <div className="download-library-toolbar"><div className="queue-search"><Icon name="search" /><input aria-label="Search downloads" placeholder="Search downloads…" value={search} onChange={e => setSearch(e.target.value)} />{search && <button aria-label="Clear download search" onClick={() => setSearch("")}><Icon name="x" /></button>}</div><div className="queue-bulk-actions"><button className="m-button" disabled={!!busy || !failed.length} onClick={() => run("retry-all", async () => { const result = await api.RetryFailed(); if (!result.added) throw new Error("These downloads are already queued."); }, "Failed downloads returned to the end of the queue.")}>Retry failed</button><button className="m-button" disabled={!!busy || !completed.length} onClick={() => run("clear", () => api.ClearCompleted(), "Completed history cleared. Your files are still in the library.")}>Clear completed</button></div></div>
    {rows.length ? <div className="download-list">{rows.map(j => <article className={`download-row ${j.status === "downloading" ? "is-running" : ""}`} key={j.id}><span className={`download-file-icon ${j.status}`}><Icon name={j.status === "done" ? "film" : "download"} /></span><div className="download-info"><div className="download-title-line"><h3>{j.title || j.url}</h3><span className={`download-status ${j.status}`}>{({ queued: "Queued", downloading: "Downloading", done: "Completed", duplicate: "Already saved", error: "Failed" } as Record<string, string>)[j.status] || j.status}</span></div><p className="download-url">{j.url}</p>{j.status === "downloading" && <><div className="download-progress" role="progressbar" aria-label={`Downloading ${j.title || j.url}`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={Math.max(0, Math.min(100, j.percent || 0))}><span style={{ width: `${Math.max(0, Math.min(100, j.percent || 0))}%` }} /></div><div className="download-meta"><span>{Math.round(j.percent || 0)}% complete</span><span>{[j.speed, j.eta ? `${j.eta} remaining` : ""].filter(Boolean).join(" · ")}</span></div></>}{j.status === "done" && <p className="download-meta">{j.replace ? "Replaced with the new download" : `${j.count || 0} file${j.count === 1 ? "" : "s"} saved to your library`}</p>}{j.status === "queued" && <p className="download-meta">{`#${position.get(j.id)} in queue${position.get(j.id) === 1 ? " · Up next" : ""}${j.replace ? " · Replacing existing file" : ""}`}</p>}{j.status === "duplicate" && <p className="download-meta">This video is already in your library.</p>}{j.error && <details className="download-error"><summary>View error details</summary><p>{j.error}</p></details>}</div>{j.status === "error" && <button className="m-button download-retry" disabled={!!busy} onClick={() => run(j.id, async () => { const result = await api.RetryFailed(j.id); if (!result.added) throw new Error("This download is already queued."); }, "Download queued again.")}>{busy === j.id ? "Retrying…" : "Retry"}</button>}{j.status === "queued" && <div className="download-order-controls" role="group" aria-label={"Queue controls for " + (j.title || j.url)}><button className="m-button" disabled={!!busy || position.get(j.id) === 1} onClick={() => run(j.id, () => api.MoveJob(j.id, "next"), "This video will download next.")}>Download next</button><div className="queue-step-controls"><button aria-label={"Move up: " + (j.title || j.url)} title="Move up" disabled={!!busy || position.get(j.id) === 1} onClick={() => run(j.id, () => api.MoveJob(j.id, "up"), "Moved up in the queue.")}><svg viewBox="0 0 20 20" aria-hidden="true"><path d="m5 12 5-5 5 5" /></svg></button><button aria-label={"Move down: " + (j.title || j.url)} title="Move down" disabled={!!busy || position.get(j.id) === waiting.length} onClick={() => run(j.id, () => api.MoveJob(j.id, "down"), "Moved down in the queue.")}><svg viewBox="0 0 20 20" aria-hidden="true"><path d="m5 8 5 5 5-5" /></svg></button></div></div>}{j.status === "queued" && <button className="download-action" aria-label={`Remove ${j.title || j.url} from queue`} disabled={!!busy} onClick={() => run(j.id, () => api.RemoveJob(j.id), "Removed from the queue.")}><Icon name="x" /></button>}</article>)}</div> : <div className="manager-empty download-empty"><span className="empty-icon"><Icon name="download" /></span><h2>{search ? "No matching downloads" : queue.length ? "Nothing here right now" : "Your queue is clear"}</h2><p>{search ? "Try a different title or link." : queue.length ? "Downloads with this status will appear here." : "Add a video link or import files to start building your library."}</p></div>}
